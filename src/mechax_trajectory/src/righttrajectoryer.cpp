...
Trajectoryer::Trajectoryer() : Node("trajectory")
{
    ...
    righttarget_sub_ = this->create_subscription<auto_aim_interfaces::msg::Target>(
                  "/righttracker/target", rclcpp::SensorDataQoS(), std::bind(&Trajectoryer::righttarget_callback, this, std::placeholders::_1));

    rightangle_sub_ = this->create_subscription<auto_aim_interfaces::msg::ReceiveSerial>(
        "/rightangle/init", 10, std::bind(&Trajectoryer::rightangle_callback, this, std::placeholders::_1));
    
    rightmaker_pub_ = this->create_publisher<visualization_msgs::msg::Marker>(
        "/rightaiming_point", 10);

    result_pub_ = this->create_publisher<auto_aim_interfaces::msg::SendSerial>(
        "/trajectory/result", 10);
    
    rightneedpose_pub_ = this->create_publisher<geometry_msgs::msg::PointStamped>(
        "/righttrajectory/needpose", rclcpp::SensorDataQoS());
...
//---------------------------------------------
    ...
    if (abs(object_x) > 0.01) {
        ...
        rightmaker_pub_->publish(aiming_point_);
    }
//---------------------------------------------
//根据需要选择模型
...
void Trajectoryer::righttarget_callback(const auto_aim_interfaces::msg::Target msg)
{
...
void Trajectoryer::rightangle_callback(const auto_aim_interfaces::msg::ReceiveSerial msg)
{
    now_pitch = msg.rightpitch / 57.3f;
    now_yaw = msg.rightyaw / 57.3f;
...
void Trajectoryer::get_need_pose(const float &object_x,const float &object_y,const float &object_z)
{
    ...
    pose.point.y = 0.0;
    pose.point.z = need_z;
    rightneedpose_pub_->publish(pose);
}
