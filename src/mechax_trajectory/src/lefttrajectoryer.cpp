...
Trajectoryer::Trajectoryer() : Node("trajectory")
{
    ...
    lefttarget_sub_ = this->create_subscription<auto_aim_interfaces::msg::Target>(
                  "/lefttracker/target", rclcpp::SensorDataQoS(), std::bind(&Trajectoryer::lefttarget_callback, this, std::placeholders::_1));
    
    leftangle_sub_ = this->create_subscription<auto_aim_interfaces::msg::ReceiveSerial>(
        "/leftangle/init", 10, std::bind(&Trajectoryer::leftangle_callback, this, std::placeholders::_1));

    leftmaker_pub_ = this->create_publisher<visualization_msgs::msg::Marker>(
        "/leftaiming_point", 10);

    result_pub_ = this->create_publisher<auto_aim_interfaces::msg::SendSerial>(
        "/trajectory/result", 10);

    leftneedpose_pub_ = this->create_publisher<geometry_msgs::msg::PointStamped>(
        "/lefttrajectory/needpose", rclcpp::SensorDataQoS());
...
//---------------------------------------------
    ...
    if (abs(object_x) > 0.01) {
        ...
        leftmaker_pub_->publish(aiming_point_);
    }
//---------------------------------------------
//根据需要选择模型
...
void Trajectoryer::lefttarget_callback(const auto_aim_interfaces::msg::Target msg)
{
...
void Trajectoryer::leftangle_callback(const auto_aim_interfaces::msg::ReceiveSerial msg)
{
    now_pitch = msg.leftpitch / 57.3f;
    now_yaw = msg.leftyaw / 57.3f;
...
void Trajectoryer::get_need_pose(const float &object_x,const float &object_y,const float &object_z)
{
    ...
    pose.point.y = 0.0;
    pose.point.z = need_z;
    leftneedpose_pub_->publish(pose);
}
